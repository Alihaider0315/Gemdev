import styles from "../talenthunt_styles/Winners.module.css";
...
export default function PrizesAndWinners() {
  ...
  const [winners, setWinners] = useState([]); // State for storing competition prizes
  ...
  const GetWinners = async () => {
    try {
      const response = await axios.get(
        process.env.NEXT_PUBLIC_BASE_URL + "/api/competition-winners"
      );
      ...
      await axios
        .get(
          ...
          process.env.NEXT_PUBLIC_BASE_URL +
            "/api/banners?s[page]=Winners&s[type]=Landing"
        )
