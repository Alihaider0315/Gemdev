  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setRows(data?.rows);
        setTeamMembers(data?.rows[data?.locations[1]?.location]);
        ...
        <div className={styles.team_sec2}>
          {Object.keys(locations)?.map((key) => (
            <div key={key}>
              <button
                onClick={(e) => {
                  setSelectedOption(locations[key]?.location);
                  if (key == 1) {
                    setTeamMembers(rows[locations[1]?.location]);
                    setSelectedCountry(null);
                  } else if (key == 6) {
                    setTeamMembers(rows[locations[6]?.location]);
                    setSelectedCountry(null);
                  } else {
                    setSelectedCountry(locations[key]?.children);

                    setSelectedCountryColor(
                      Object.values(locations[key]?.children)[0]?.location
                    );
                    setTeamMembers(
                      rows[Object.values(locations[key]?.children)[0]?.location]
                    );
                  }
                }}
                className={
